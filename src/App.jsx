...
export default function App() {
  ...
  const [isBookLoading, setIsBookLoading] = useState(false); // New state for book-specific loading
  ...
  const handleSelectBook = (book) => {
    ...
    setIsBookLoading(true); // Start loading spinner for the book
    setBookContent(null); // Clear old book content immediately
    fetch(`/content/${book.contentFile}`, { cache: 'no-cache' })
      ...
      .then(data => {
        ...
        setIsBookLoading(false); // Stop loading spinner on success
      })
      .catch(error => {
        console.error("Failed to load book content:", error);
        setIsBookLoading(false); // Stop loading spinner on failure
  ...
  // This is the main view logic. It now handles all loading states.
  const renderContent = () => {
    if (isLoading) {
      return <LoadingScreen message="Loading Library..." />;
    }
    if (selectedBook) {
      if (isBookLoading || !bookContent) {
        // THIS IS THE CRITICAL FIX: Show a loading screen while the book is being fetched.
        // It will never try to render ReadingView with null data.
  ...
  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
    {library.map(book => (
      <div key={book.id} onClick={() => onSelectBook(book)} className="cursor-pointer group">
        <img src={book.coverImage} alt={`Cover of ${book.title}`} className="rounded-lg shadow-lg group-hover:shadow-xl transform group-hover:-translate-y-1 transition-all duration-300 w-full" />
        <div className="mt-2">
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-150px)]">
      ...
      <div ref={contentPaneRef} className="flex-grow overflow-hidden relative w-full">
          <div ref={contentTextRef} className="h-full text-lg leading-relaxed font-serif p-4 md:p-6 text-left" style={{ height: '100%', columnWidth: contentPaneRef.current ? `${contentPaneRef.current.clientWidth}px` : '100%', columnGap: '50px', transform: `translateX(-${(currentPage - 1) * (contentPaneRef.current?.clientWidth + 50 || 0)}px)`, transition: 'transform 0.4s ease-in-out' }}>
             {chapter.content.map((paragraph, index) => (<p key={index} className="mb-6 break-inside-avoid-column">{paragraph}</p>))}
          </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-30 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-2xl" onClick={e => e.stopPropagation()}>
        ...
        {isLoading ? ( <div className="flex items-center justify-center h-40"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500"></div></div>) : (<p className="text-base leading-relaxed">{response}</p>)}
