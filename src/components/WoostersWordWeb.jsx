...
/** ---------- Safe default 14x14 puzzle ---------- */
const DEFAULT_PUZZLE = {
  theme: "Wodehouse Sampler",
  size: 14,
  grid: Array.from({ length: 14 }, (_, y) =>
    Array.from({ length: 14 }, (_, x) =>
      String.fromCharCode(65 + ((x * 13 + y * 7) % 26))
    ).join("")
  ),
  answers: ["WOOSTER", "JEEVES", "BLANDINGS", "SCRIPTURE", "WHISKY", "AGATHA"],
};
...
function HowModal({ open, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl p-5"
      ...
      >
        ...
        <ol className="list-decimal pl-5 space-y-2 text-sm leading-relaxed">
          <li>
            <b>Find 7 themed entries</b> hidden in the letter grid. Words can run{" "}
            <i>horizontally, vertically, diagonally, and may bend</i>. Reverse is
            allowed.
          </li>
          <li>
            <b>Drag to select</b> letters (mouse or touch). You can{" "}
            <b>backtrack</b>: sliding back over the previous tile removes it.
          </li>
          <li>
            Release to submit; if the path matches an answer (forwards or
            backwards), it’s marked as found.
          </li>
          <li>
            Stuck? Tap <b>Hint</b> for a gentle nudge. Inactivity will give a
            subtle pulse on the theme area.
          </li>
          <li>
            <b>Reset</b> clears found words and current selection. Today’s puzzle
            rotates daily (America/Chicago).
          </li>
        </ol>
      </div>
    </div>
                ...
                return (
                  <div
                    ...
                    style={{
                      width: cell,
                      height: cell,
                      fontSize: Math.max(12, Math.floor(cell * 0.55)),
                    }}
                    ...
                    onTouchMove={(e) => {
                      ...
                      if (!attr) return;
                      const [xx, yy] = attr.split(",").map((n) => parseInt(n));
                      extendSelect(xx, yy);
                    }}
                  >
                    {ch}
